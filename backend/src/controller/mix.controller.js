...
const groupMixes = (mixes) => {
  return mixes.reduce(
    (acc, mix) => {
      if (mix.type === "Genre") acc.genreMixes.push(mix);
      else if (mix.type === "Mood") acc.moodMixes.push(mix);
      return acc;
    },
...
export const updateDailyMixes = async () => {
  try {
    ...
    for (const source of sources) {
      ...
      const mixKey = `mixes.${source.type.toLowerCase()}.${source.name.toLowerCase().replace(/\s+/g, "_")}`;
...
export const getDailyMixes = async (req, res, next, returnInternal = false) => {
  try {
    ...
    if (req.user && req.user.id) {
      const listenHistory = await ListenHistory.find({ user: req.user.id })
        .limit(150)
      ...
      if (validHistory.length > 0) {
        ...
        validHistory.forEach((item) => {
          (item.song.genres || []).forEach(
            (g) => (preferenceCounts[g] = (preferenceCounts[g] || 0) + 1),
          );
          (item.song.moods || []).forEach(
            (m) => (preferenceCounts[m] = (preferenceCounts[m] || 0) + 1),
          );
        });
        mixes.sort(
          (a, b) =>
            (preferenceCounts[b.sourceId.toString()] || 0) -
            (preferenceCounts[a.sourceId.toString()] || 0),
        );
      }
    }
    ...
    if (returnInternal) return groupedData;
    return res.status(200).json(groupedData);
  } catch (error) {
    if (returnInternal) return { genreMixes: [], moodMixes: [] };
    next(error);
  }
};
